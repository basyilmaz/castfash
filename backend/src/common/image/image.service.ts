...
export class ImageService {
    ...
    private async initializeSharp(): Promise<void> {
        try {
            ...
            this.logger.warn('⚠️ Sharp not available. Image optimization disabled. Install with: npm install sharp');
    ...
    async getMetadata(imagePath: string): Promise<{
        ...
        try {
            ...
            this.logger.warn(`Could not get metadata for ${imagePath}: ${error.message}`);
            return { size: stats.size };
        }
    }
    ...
    async optimize(
        ...
        try {
            ...
            const optimizedPath = outputPath || join(dir, `${baseName}_optimized${outputExt}`);
            ...
            if (generateThumbnail) {
                ...
                await this.sharp.default(inputPath)
