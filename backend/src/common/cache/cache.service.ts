  ...
  async onModuleInit() {
    ...
    if (redisUrl) {
      try {
        ...
        this.logger.warn(
          `Redis not available: ${error.message}. Using in-memory cache.`,
        );
        this.isRedisEnabled = false;
      }
    } else {
  ...
  async get<T>(key: string): Promise<T | null> {
    if (this.isRedisEnabled && this.redisClient) {
      try {
        ...
        this.logger.warn(
          `Redis get error for key ${key}, falling back to memory`,
        );
      }
    }
  ...
  async set<T>(key: string, value: T, ttlSeconds?: number): Promise<void> {
    ...
    if (this.isRedisEnabled && this.redisClient) {
      try {
        ...
        this.logger.warn(
          `Redis set error for key ${key}, falling back to memory`,
        );
      }
    }
